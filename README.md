...
Set your normal desktop resolution (the one you use outside Valorant). Example: `2560 x 1440` for a 1440p display.

### Step 3 -- Stretch Resolution
Set the resolution you want in Valorant. Example: `2100 x 1440` gives a ~4:3 stretched look on a 1440p display.
...
The app uses **WMI process events** (zero polling overhead) to detect Valorant. When WMI is unavailable, it falls back to 3-second polling. A watchdog checks every 8 seconds that the resolution has not drifted (Valorant's anti-cheat can reset it).
...
For NVIDIA GPU users -- create custom resolutions that are not in Windows' default list.
...
**Resolution does not switch when Valorant launches**
...
**App does not start with Windows after moving the exe**
